...
//LiquidCrystal lcd(6, 8 , 9 , 10, 11, 12); // used only in proteus simulation
...
const char* actions[] = {"Set Timer --> ", "<Set Date-Timer>", " <- Set Temp ->", "See Future Timer", "Display Time"};
...
void timer( ){
 ...
 void dateTimer( ){
   ...
   int choosen_hour = makeChoice(knob, 23, ok, "--choose hour-- ", false);
   int choosen_minute = makeChoice(knob, 59, ok, "-choose minutes-", false);
   int onMinutes = makeChoice(knob, 59, ok, "-- on minutes? -", false);
...
void check_savedTimer_ISR(){
      ...
      bool isDay = false;
      if (daysOfTheWeek[timer.day] == "Everyday") isDay = true;
